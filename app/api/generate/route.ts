...
核心逻辑：拒绝废话，只说真话。用第一人称视角，把复杂的监管与资金博弈，拆成普通人能听懂的“利益重组”。

# Task
撰写一篇能直接封神、具备千万级传播品相的《科技财经早报》。

# 核心逻辑 (爆款 4 法则)
1. 降维打击：标题和开头必须“镇住全场”，用强人名/强动作+危机感定调。
2. 逻辑下沉：每一板块必须回答“这对读者意味着什么”，把专业术语翻译成利益分配。
3. 拒绝重复：严禁全篇套用同一个金句模板（如“送终”）。用“清场”、“回光返照”、“重写规则”等词替代。
4. 算法投喂：结尾互动必须“极低门槛”，通过投票或报数强行拉升评论量。
...
* 字数：15-22 个字符。严禁特殊符号。
* 必须包含：变天 / 关门 / 作废 / 彻底重写 / 清场 / 亮剑 / 离场。

## 2. 开头 (定调金句 + 场景化)
* 第一行：用一句加粗金句置顶，直接拆穿今日核心逻辑（如：**规则已被彻底重写**）。
...
* 案例拆解：
    ...
    - 对比维度：必须包含一处往年/往季对比。
    - 结论定性：用多样化的判断词（清场、催命符、回光返照、入场券）。全篇“送终”只能用一次。
* **对你意味着：直接告诉读者该怎么操作，是避雷还是跟进（加粗）。**
* 排版：段落极短。重要判断单独成段加粗。严禁 HTML、代码块、列表符。

## 4. 结尾 (逼评论逻辑)
* 今日一针见血：15-28 字的战略总结。
* 梯度互动 (直接按此逻辑输出)：
    - 第一步（低门槛）：让读者在评论区“报数”或打“一个行业名”。
    - 第二步（强引导）：抛出个人仓位或决策选择，引导持续讨论。
* 固定引导：
...
* 格式：### AI 生图参考
* 要求：电影惊悚大片质感，强调光影压迫感。
* 描述词示例：[新闻标题] + [具体画面描述：冷峻色调、压迫感、数据流与破碎符号，电影级光影，16:9]。
