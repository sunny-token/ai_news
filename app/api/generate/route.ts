...
const generateArticle = async (newsList: HotNewsItem[]): Promise<string> => {
  ...
  const prompt = `
OldSun大喇叭·科技财经早报Prompt（贴合成品文章版）
...
排版别拖沓，每段不超过3行，关键判断、操作指引单独加粗，语言口语化但不低俗，符合操盘手的干练劲儿，正文之间可加短句衔接（如“说完XX，再看另一个更致命的信号”），避免脱节。
...
##### （二）文章内图提示词（仅1张，开头下方、正文上方专用，16:9）
...
关键词别重复，“清场”“变天”等判断词交替使用，用“洗牌”“淘汰”“崩塌”“回光返照”“催命符”等替代，避免频繁重复，贴合成品文章的用词逻辑。
